...
### 20. Sistema de Roles y Permisos (nuevo)

| Funcionalidad                           | Estado | Descripción                                                                  |
| --------------------------------------- | ------ | ---------------------------------------------------------------------------- |
| **Jerarquía Heredable**                 | Listo  | Admin (100) > Pastor (80) > Líderes (60/40) > Voluntario (20) > Miembro (10) |
| **Permisos Contextuales**               | Listo  | Verificación por Red, Grupo o Área de servicio específica                    |
| **Core de Permisos (`permissions.ts`)** | Listo  | Centralización de 50+ permisos con lógica de negocio desacoplada             |
| **Helpers de Servidor**                 | Listo  | `requirePermission` y `requireAdmin` para Server Actions                     |
| **PermissionGuard (Client)**            | Listo  | Componente para ocultar UI basado en permisos/roles                          |
| **Carga de Roles en Sesión**            | Listo  | NextAuth incluye roles y relaciones en la sesión JWT                         |
| **Módulo de Áreas de Servicio**         | Listo  | CRUD de áreas (Alabanza, Ujieres, etc.) con líderes y voluntarios            |
...
| Archivo                                     | Cambio                                                                                          |
| ------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| `src/prisma/schema.prisma`                  | Agregados modelos `ServiceArea`, `ServiceAreaLeader`, `VolunteerAssignment` y enum `SystemRole` |
| `src/libs/permissions.ts`                   | **NUEVO** - Lógica central de permisos y jerarquías                                             |
| `src/libs/permissions-server.ts`            | **NUEVO** - Helpers para validación en Server Actions                                           |
| `src/components/guards/PermissionGuard.tsx` | **NUEVO** - Guard para componentes del lado del cliente                                         |
| `src/app/server/roleActions.ts`             | **NUEVO** - Gestión de roles y promociones de liderazgo                                         |
| `src/app/server/areaActions.ts`             | **NUEVO** - CRUD y gestión de voluntarios en áreas de servicio                                  |
| `src/libs/auth.ts`                          | Integración de roles y relaciones en la sesión de NextAuth                                      |
| `src/types/next-auth.d.ts`                  | Aumentado tipo de sesión para incluir roles y datos contextuales                                |

### Módulo de Calendario (anterior)
