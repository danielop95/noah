...
export const SettingsProvider = (props: Props) => {
  // Usar color del tenant si existe, sino el default
  const defaultPrimaryColor = props.tenantPrimaryColor || primaryColorConfig[0].main
  ...
  // State - usar color del tenant si existe, ignorando la cookie para primaryColor
  const initialStateFromCookie = JSON.stringify(settingsCookie) !== '{}' ? settingsCookie : updatedInitialSettings

  const [_settingsState, _updateSettingsState] = useState<Settings>({
    ...initialStateFromCookie,
    // El color del tenant siempre prevalece sobre la cookie
    primaryColor: props.tenantPrimaryColor || initialStateFromCookie.primaryColor
  })
